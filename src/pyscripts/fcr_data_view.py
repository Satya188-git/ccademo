...
QueryString = f"""CREATE OR REPLACE VIEW {view_name} AS (
    SELECT customer_endpoint_address,
     a.contact_id,
     queue_enqueue_timestamp,
     agent_username,
     queue_name requested_call_type,
     (CASE WHEN (actual_call_type IS NULL) THEN queue_name ELSE actual_call_type END) actual_call_type,
     date_parse(substr(CAST(initiation_timestamp AS varchar), 1, 19), '%Y-%m-%d %H:%i:%s') initiation_timestamp_original,
     date_parse(substr(CAST(date_add('second', (-(agent_interaction_duration_ms) / 1000), disconnect_timestamp) AS varchar), 1, 19), '%Y-%m-%d %H:%i:%s') initiation_timestamp,
     date_parse(substr(CAST(disconnect_timestamp AS varchar), 1, 19), '%Y-%m-%d %H:%i:%s') disconnect_timestamp,
     (CASE WHEN (Lead(date_add('second', (-(agent_interaction_duration_ms) / 1000), disconnect_timestamp)) OVER (PARTITION BY customer_endpoint_address, queue_name ORDER BY date_add('second', (-(agent_interaction_duration_ms) / 1000), disconnect_timestamp) ASC) IS NULL) THEN 9999999999 ELSE date_diff('second', disconnect_timestamp, Lead(date_add('second', (-(agent_interaction_duration_ms) / 1000), disconnect_timestamp)) OVER (PARTITION BY customer_endpoint_address, queue_name ORDER BY date_add('second', (-(agent_interaction_duration_ms) / 1000), disconnect_timestamp) ASC)) END) time_difference_seconds,
     substr(CAST(initiation_timestamp AS varchar), 1, 4) year
    FROM
    ...
    WHERE ((queue_name IS NOT NULL) AND (date(initiation_timestamp) >= date('2024-01-01')) AND (agent_interaction_duration_ms > 0));""",
