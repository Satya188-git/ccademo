...
resource "aws_glue_catalog_table" "shared_connect_contact_flow_events" {
  name          = var.source_table_names[3] # Name for the resource link table
  database_name = aws_glue_catalog_database.glue_data_catalog_connect_datalake.name
  lifecycle {
    ignore_changes = all
  }
  ...
  target_table {
    catalog_id    = var.producer_catalog_id   # Replace with the AWS account ID where the original table resides
    database_name = var.source_database_name  # The original Glue database name in the other account
    name          = var.source_table_names[3] # The original table name in the shared Glue database
  }
}

resource "aws_glue_catalog_table" "shared_connect_contact_evaluation_record" {
  name          = var.source_table_names[2] # Name for the resource link table
  database_name = aws_glue_catalog_database.glue_data_catalog_connect_datalake.name
  lifecycle {
    ignore_changes = all
  }
  ...
  target_table {
    catalog_id    = var.producer_catalog_id   # Replace with the AWS account ID where the original table resides
    database_name = var.source_database_name  # The original Glue database name in the other account
    name          = var.source_table_names[2] # The original table name in the shared Glue database
  }
}

resource "aws_glue_catalog_table" "shared_connect_agent_queue_statistic_record" {
  name          = var.source_table_names[1] # Name for the resource link table
  database_name = aws_glue_catalog_database.glue_data_catalog_connect_datalake.name
  lifecycle {
    ignore_changes = all
  }
  ...
  target_table {
    catalog_id    = var.producer_catalog_id   # Replace with the AWS account ID where the original table resides
    database_name = var.source_database_name  # The original Glue database name in the other account
    name          = var.source_table_names[1] # The original table name in the shared Glue database
  }
}

resource "aws_glue_catalog_table" "shared_connect_agent_statistic_record" {
  name          = var.source_table_names[0] # Name for the resource link table
  database_name = aws_glue_catalog_database.glue_data_catalog_connect_datalake.name
  lifecycle {
    ignore_changes = all
  }
  ...
  target_table {
    catalog_id    = var.producer_catalog_id   # Replace with the AWS account ID where the original table resides
    database_name = var.source_database_name  # The original Glue database name in the other account
    name          = var.source_table_names[0] # The original table name in the shared Glue database
  }
}